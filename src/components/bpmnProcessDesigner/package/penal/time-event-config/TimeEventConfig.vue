<template>
  <div class="panel-tab__content">
    <div style="margin-top: 10px">
      <span>类型：</span>
      <n-button-group>
        <n-button size="tiny" :type="type === 'time' ? 'primary' : 'default'" @click="setType('time')">
          时间
        </n-button>
        <n-button
          size="tiny"
        ...
        <n-button size="tiny" :type="type === 'cycle' ? 'primary' : 'default'" @click="setType('cycle')">
          循环
        </n-button>
      </n-button-group>
      <n-icon v-if="valid" color="green" :component="CheckmarkCircle" style="margin-left: 8px" />
    </div>
    <div style="margin-top: 10px; display: flex; align-items: center">
      ...
      >
        <template #suffix>
          <n-tooltip v-if="!valid" content="格式错误" placement="top">
            <template #trigger>
              <n-icon color="orange" :component="Warning" />
            </template>
          </n-tooltip>
          <n-tooltip :content="helpText" placement="top">
            <template #trigger>
              <n-icon
...
import { CheckmarkCircle, Warning, HelpCircle } from '@vicons/ionicons5'
