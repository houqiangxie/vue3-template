<template>
  <n-tabs v-model:value="tab">
    <n-tab-pane label="CRON表达式" name="cron">
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 8px">
        <n-input v-model:value="fields.second" placeholder="秒" style="width: 80px" :key="'second'" />
        <n-input v-model:value="fields.minute" placeholder="分" style="width: 80px" :key="'minute'" />
        <n-input v-model:value="fields.hour" placeholder="时" style="width: 80px" :key="'hour'" />
        <n-input v-model:value="fields.day" placeholder="天" style="width: 80px" :key="'day'" />
        <n-input v-model:value="fields.month" placeholder="月" style="width: 80px" :key="'month'" />
        <n-input v-model:value="fields.week" placeholder="周" style="width: 80px" :key="'week'" />
        <n-input v-model:value="fields.year" placeholder="年" style="width: 80px" :key="'year'" />
      </div>
      <n-tabs v-model:value="activeField" type="card" style="margin-bottom: 8px">
        <n-tab-pane v-for="f in cronFieldList" :label="f.label" :name="f.key" :key="f.key">
          <div style="margin-bottom: 8px">
            <n-radio-group v-model:value="cronMode[f.key]" :key="'radio-' + f.key">
              <n-radio label="every" :key="'every-' + f.key">每{{ f.label }}</n-radio>
              <n-radio label="range" :key="'range-' + f.key"
                >从
                <n-input-number
                ...
                之间每{{ f.label }}</n-radio
              >
              <n-radio label="step" :key="'step-' + f.key"
                >从第
                <n-input-number
                ...
                {{ f.label }}</n-radio
              >
              <n-radio label="appoint" :key="'appoint-' + f.key">指定</n-radio>
            </n-radio-group>
          </div>
          <div v-if="cronMode[f.key] === 'appoint'">
            <n-checkbox-group v-model:value="cronAppoint[f.key]" :key="'group-' + f.key">
              <n-checkbox
                v-for="n in f.max + 1"
                :label="pad(n - 1)"
                :key="'cb-' + f.key + '-' + (n - 1)"
    ...
    <n-tab-pane label="标准格式" name="iso" :key="'iso-tab'">
      <div style="margin-bottom: 10px">
        <n-input
      ...
      <div style="margin-bottom: 10px"
        >循环次数：<n-input-number v-model:value="repeat" :min="1" style="width: 100px" :key="'repeat'"
      /></div>
      <div style="margin-bottom: 10px"
        >日期时间：<n-date-picker
