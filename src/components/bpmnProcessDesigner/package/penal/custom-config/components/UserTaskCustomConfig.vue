...
<template>
  <div>
    <n-divider content-position="left">审批类型</n-divider>
    <n-form-item path="approveType">
      <n-radio-group v-model:value="approveType.value">
        <n-radio
    ...
    <n-form-item
      ...
      path="returnNodeId"
    >
      <n-select v-model:value="returnNodeId" clearable style="width: 100%" @change="updateReturnNodeId" :options="returnTaskList.map(item => ({ label: item.name, value: item.id }))" />
    </n-form-item>

    <n-divider content-position="left">审批人为空时</n-divider>
    <n-form-item path="assignEmptyHandlerType">
      <n-radio-group v-model:value="assignEmptyHandlerType" @change="updateAssignEmptyHandlerType">
        <div class="flex-col">
    ...
    <n-form-item
      ...
      label="指定用户"
      path="assignEmptyHandlerUserIds"
      span="24"
    >
      <n-select
       ...
       :options="userOptions.map(item => ({ label: item.nickname, value: item.id }))" />
    </n-form-item>

    <n-divider content-position="left">审批人与提交人为同一人时</n-divider>
    <n-radio-group v-model:value="assignStartUserHandlerType" @change="updateAssignStartUserHandlerType">
      <div class="flex-col">
    ...
    <n-divider content-position="left">操作按钮</n-divider>
    <div class="button-setting-pane">
      ...
      <div class="button-setting-item" v-for="(item, index) in buttonsSettingEl" :key="index">
        ...
        <div class="button-setting-item-label">
          ...
          />
          <n-button v-else text @click="changeBtnDisplayName(index)"
            >{{ item.displayName }} &nbsp;<Icon icon="ep:edit"
          /></n-button>
        </div>
        <div class="button-setting-item-label">
          <n-switch v-model:value="item.enable" @change="updateElementExtensions" />
        </div>
      </div>
    </div>

    <n-divider content-position="left">字段权限</n-divider>
    ...
    <n-form-item path="signEnable">
      <n-switch
        v-model:value="signEnable.value"
        @change="updateElementExtensions"
      />
    </n-form-item>
    ...
    <n-form-item path="reasonRequire">
      <n-switch
        v-model:value="reasonRequire.value"
        @change="updateElementExtensions"
      />
    </n-form-item>
  </div>
...
.button-setting-pane {
  ...
  .button-setting-title {
    ...
    height: 45px;
    padding-left: 12px;
    background-color: var(--bpmn-hover, #f8fafc0a);
    border: 1px solid var(--bpmn-border, #1f38581a);
  ...
  .button-setting-item {
    ...
    .editable-title-input {
      ...
      margin-left: 4px;
      line-height: 24px;
      border: 1px solid var(--bpmn-border, #d9d9d9);
      border-radius: 4px;
      background: var(--bpmn-card, transparent);
      color: var(--bpmn-text, inherit);
      transition: all 0.3s;

      &:focus {
        border-color: var(--bpmn-primary, #40a9ff);
        outline: 0;
        box-shadow: 0 0 0 2px color-mix(in srgb, var(--bpmn-primary, #1890ff) 20%, transparent);
      }
    }
  }
}
