  ...
  <n-form label-width="120px">
    <n-form-item label="规则类型" path="candidateStrategy">
      <n-select v-model:value="userTaskForm.candidateStrategy" clearable style="width: 100%"
        @change="changeCandidateStrategy" :options="CANDIDATE_STRATEGY" />
    </n-form-item>
    <n-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.ROLE" label="指定角色" path="candidateParam">
      <n-select v-model:value="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask" :options="roleOptions.map(item => ({ label: item.name, value: item.id }))" />
    </n-form-item>
    <n-form-item v-if="
    ...
    " label="指定部门" path="candidateParam" span="24">
      <n-tree-select
        ref="treeRef"
    ...
    <!-- <n-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.POST" label="指定岗位" path="candidateParam"
      span="24">
      <n-select v-model:value="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask" :options="postOptions.map(item => ({ label: item.name, value: item.id }))" />
    </n-form-item> -->
    <n-form-item v-if="userTaskForm.candidateStrategy == CandidateStrategy.USER" label="指定用户" path="candidateParam"
      span="24">
      <n-select v-model:value="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask" :options="userOptions.map(item => ({ label: item.nickname, value: item.id }))" />
    </n-form-item>
    <n-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.USER_GROUP" label="指定用户组"
      path="candidateParam">
      <n-select v-model:value="userTaskForm.candidateParam" clearable multiple style="width: 100%"
        @change="updateElementTask" :options="userGroupOptions.map(item => ({ label: item.name, value: item.id }))" />
    </n-form-item>
    <n-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.FORM_USER" label="表单内用户字段" path="formUser">
      <n-select v-model:value="userTaskForm.candidateParam" clearable style="width: 100%" @change="handleFormUserChange" :options="userFieldOnFormOptions.map(item => ({ label: item.title, value: item.field }))" />
    </n-form-item>
    <n-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.FORM_DEPT_LEADER" label="表单内部门字段"
      path="formDept">
      <n-select v-model:value="userTaskForm.candidateParam" clearable style="width: 100%" @change="updateElementTask" :options="deptFieldOnFormOptions.map(item => ({ label: item.title, value: item.field }))" />
    </n-form-item>
    <n-form-item v-if="
      userTaskForm.candidateStrategy == CandidateStrategy.MULTI_LEVEL_DEPT_LEADER ||
    ...
    " :label="deptLevelLabel!" path="deptLevel" span="24">
      <n-select v-model:value="deptLevel" clearable @change="updateElementTask" :options="MULTI_LEVEL_DEPT" />
    </n-form-item>
    <n-form-item v-if="userTaskForm.candidateStrategy === CandidateStrategy.EXPRESSION" label="流程表达式"
      path="candidateParam">
    ...
    <n-form-item label="跳过表达式" path="skipExpression">
      <n-input type="textarea" v-model:value="userTaskForm.skipExpression" clearable style="width: 100%"
...
onMounted(async () => {
  ...
  deptTreeOptions.value = toNaiveTreeSelectOptions(handleTree(depts, 'id'))
  // 获得岗位列表
