<template>
  <div
    ...
    :style="{ ...bpmnThemeStyle, width: `${width}px`, maxHeight: '600px' }"
  >
    <n-collapse v-if="isReady" accordion v-model:expanded-names="activeTab">
      ...
      <n-collapse-item name="condition" v-if="elementType === 'Process'" key="message">
        <template #title><Icon icon="ep:comment" />消息与信号</template>
        <signal-and-massage />
      </n-collapse-item>
      <n-collapse-item name="condition" v-if="conditionFormVisible" key="condition">
        <template #title><Icon icon="ep:promotion" />流转条件</template>
        <flow-condition :business-object="elementBusinessObject" :type="elementType" />
      </n-collapse-item>
      <n-collapse-item name="condition" v-if="formVisible" key="form">
        <template #title><Icon icon="ep:list" />表单</template>
        <element-form :id="elementId" :type="elementType" />
      </n-collapse-item>
      <n-collapse-item name="task" v-if="isTaskCollapseItemShow(elementType)" key="task">
        <template #title
      ...
      <n-collapse-item name="listeners" key="listeners">
        <template #title><Icon icon="ep:bell-filled" />执行监听器</template>
        <element-listeners :id="elementId" :type="elementType" />
      </n-collapse-item>
      <n-collapse-item name="taskListeners" v-if="elementType === 'UserTask'" key="taskListeners">
        <template #title><Icon icon="ep:bell-filled" />任务监听器</template>
        <user-task-listeners :id="elementId" :type="elementType" />
      </n-collapse-item>
      <n-collapse-item name="extensions" key="extensions">
        <template #title><Icon icon="ep:circle-plus-filled" />扩展属性</template>
        <element-properties :id="elementId" :type="elementType" />
      </n-collapse-item>
      <n-collapse-item name="other" key="other">
        <template #title><Icon icon="ep:promotion" />其他</template>
        <element-other-config :id="elementId" />
      </n-collapse-item>
      <n-collapse-item name="customConfig" key="customConfig">
      ...
      <!-- 新增的时间事件配置项 -->
      <n-collapse-item v-if="elementType === 'IntermediateCatchEvent'" name="timeEvent">
