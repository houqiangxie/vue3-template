 ...
 * @LastEditTime: 2022-12-27 10:33:59
-->
<template>
    <div class="w-full p-5 search-box bg-white relative rounded-t-lg">
        ...
        <div :class="`box grid grid-cols-${cols} gap-x-5 gap-y-3`">
            <n-form-item :label="item.label||item.title"  v-for="(item,index) in config" :key="index"  v-show="showAllSearchField||item.isSearch" v-bind="item.bindItem">
              <slot v-if="item.bind?.showSlot">
                    <slot :name="item.bind.slotName" :row="item"></slot>
              </slot>
            <template v-else-if="typeof item.key=='string'">
                <component  class="w-full" :is="componentList[item.component||'NInput']" v-model:value="searchModel[item.component == 'NDatePicker'?(item.key+'value'):item.key]" v-model:formatted-value="searchModel[item.key]"  :options="item.options" :type="item.type" :clearable="true" v-bind="item.bind" v-on="{...item.on}"></component>
            </template>
            <template v-else>
                    <n-grid x-gap="12" :cols="item.key&&item.key.length">
                        <n-form-item-gi v-for="(it,i) in item.key" :key="i" >
                            <component class="w-full" :is="componentList[item.component&&item.component[i]||'NInput']" :type="item.type&&item.type[i]||'input'" v-model:value="searchModel[item.component == 'NDatePicker' ? (it + 'value') :it]"  :clearable="true" :options="item.options?.[i]" v-model:formatted-value="searchModel[it]" v-bind="item.bind?.[i]||{}" v-on="{...(item.on?.[i]||{})}"></component>
                        </n-form-item-gi>
                    </n-grid>
            </template>
            </n-form-item>
            <n-form-item v-if="showSearchButton" v-bind="bindSearch" class="search-btn">
                <n-button class="mr-2" type="info" @click="getList"><svg class="icon text-white text-xs mr-1" aria-hidden="true"><use xlink:href="#icon-search"></use></svg>查询</n-button>
