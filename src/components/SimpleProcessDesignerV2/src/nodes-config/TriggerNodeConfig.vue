  ...
  <n-drawer v-model:show="settingVisible"
    ...
    <div>
      <n-form ref="formRef" :model="configForm" label-placement="top" :rules="formRules">
        <n-form-item label="触发器类型" path="type">
          <n-select v-model:value="configForm.type" @change="changeTriggerType" :options="TRIGGER_TYPES" />
        </n-form-item>
        <!-- HTTP 请求触发器 -->
        <div
          v-if="
        ...
        <div v-if="configForm.type === TriggerTypeEnum.FORM_UPDATE">
          <div v-for="(formSetting, index) in configForm.formSettings" :key="index">
            <n-card class="w-580px mt-4">
              ...
              <n-divider content-position="left">修改表单字段设置</n-divider>
              <!-- 表单字段修改设置 -->
              <div
                ...
                <div class="mr-2 flex items-center">
                  <n-form-item>
                    <n-select
                      class="w-160px!"
                      :value="key"
                      :options="optionalUpdateFormFieldOptions"
                      :disabled="key !== ''"
                      placeholder="请选择表单字段"
                      @update:value="(newKey) => updateFormFieldKey(formSetting, key, newKey as string)"
                    />
                  </n-form-item>
                </div>
                <div class="mx-2"><n-form-item>的值设置为</n-form-item></div>
                <div class="mr-2">
                  <n-form-item
                    :path="`formSettings.${index}.updateFormFields.${key}`"
                    :rules="{
                      required: true,
                      message: '值不能为空',
                      trigger: 'blur'
              ...
              <!-- 添加表单字段按钮 -->
              <n-button type="primary" text @click="addFormFieldSetting(formSetting)">
          ...
          <!-- 添加新的设置 -->
          <n-button class="mt-6" type="primary" text @click="addFormSetting">
        ...
        <div v-if="configForm.type === TriggerTypeEnum.FORM_DELETE">
          <div v-for="(formSetting, index) in configForm.formSettings" :key="index">
            <n-card class="w-580px mt-4">
              ...
              <n-divider content-position="left">删除表单字段设置</n-divider>
              <!-- 表单字段删除设置 -->
              <div class="flex flex-wrap gap-2">
                <n-select
                  ...
                  class="w-full"
                 :options="formFields.map(field => ({ label: field.title, value: field.field }))" />
              </div>
            </n-card>
          </div>
      ...
      <n-space justify="end">
        ...
        <n-button @click="cancelConfig">取 消</n-button>
      </n-space>
    </template>
    </n-drawer-content>
  </n-drawer>
...
const optionalUpdateFormFieldOptions = computed(() =>
  optionalUpdateFormFields.value.map((field) => ({
    label: field.title,
    value: field.field,
