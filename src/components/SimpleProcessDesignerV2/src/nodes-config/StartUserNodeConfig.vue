    ...
    <n-tabs type="border-card" v-model:value="activeTabName">
      <n-tab-pane tab="权限" name="user">
        ...
        <div v-else-if="startUserIds && startUserIds.length > 0">
          <span v-if="startUserIds.length == 1">
            {{ getUserNicknames(startUserIds) }} 可发起流程
          </span>
          <span v-else>
            <n-tooltip class="box-item" placement="top" :content="getUserNicknames(startUserIds)">
<template #trigger>
        ...
        <div v-else-if="startDeptIds && startDeptIds.length > 0">
          <span v-if="startDeptIds.length == 1">
            {{ getDeptNames(startDeptIds) }} 可发起流程
          </span>
          <span v-else>
            <n-tooltip class="box-item" placement="top" :content="getDeptNames(startDeptIds)">
<template #trigger>
