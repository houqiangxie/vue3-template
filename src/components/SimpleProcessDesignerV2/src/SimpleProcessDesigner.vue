<template>
  <div class="simple-process-designer">
    <n-spin class="simple-process-designer__spin" :show="loading">
      <SimpleProcessModel
...
const validateNode = (node: SimpleFlowNode | undefined, nodes: SimpleFlowNode[]) => {
  if (node) {
    ...
    if (type == NodeType.START_USER_NODE) {
      ...
      validateNode(node.childNode, nodes)
    }
...
/** 校验并同步当前简易流程到父级 */
const validateAndSync = async (): Promise<SimpleFlowNode> => {
  const nodes: SimpleFlowNode[] = []
  validateNode(processNodeTree.value, nodes)
  if (nodes.length > 0) {
    errorNodes.value = nodes
  ...
  await saveSimpleFlowModel(processNodeTree.value)
...
onMounted(async () => {
  try {
    ...
    deptTreeOptions.value = toNaiveTreeSelectOptions(
      handleTree(deptOptions.value as DeptApi.DeptVO[], 'id'),
    )
    // 获取用户组列表
...
.simple-process-designer__spin {
  flex: 1;
  min-height: 0;
  height: 100%;
}
.simple-process-designer__spin :deep(.n-spin-container),
.simple-process-designer__spin :deep(.n-spin-content) {
  height: 100%;
  min-height: 0;
}
</style>
