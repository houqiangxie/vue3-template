...
/** 上级菜单下拉�?*/
export function menuToTreeOptions(flatList: SysMenu[], excludeId?: number) {
  const roots = flatList.filter(m => m.parentId === 0)
...
/** 角色权限分配�?*/
export function menuToTreeSelectData(menus: SysMenu[]) {
  return menus.map(m => ({
    key: m.menuId,
    label: m.menuName,
