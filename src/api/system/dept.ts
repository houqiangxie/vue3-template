...
/** 上级部门下拉树 */
export function deptToTreeOptions(flatList: SysDept[], excludeId?: number): DeptTreeOption[] {
  function buildOptions(parentId: number): DeptTreeOption[] {
    return flatList
...
/** 排除子节点的部门树选项 */
export function deptToTreeSelectData(depts: SysDept[]): DeptTreeOption[] {
  return depts.map(d => ({
    key: d.deptId,
    label: d.deptName,
