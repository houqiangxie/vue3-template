...
export default class CesiumMeasure {
   ...
   */
  transformWGS84ArrayToCartesianArray(WSG84Arr: any, alt: any) {
   ...
   */
  transformCartesianArrayToWGS84Array(cartesianArr: any) {
    if (this._viewer) {
      var $this = this;
      return cartesianArr
   ...
   * @param {*} positions
   */
  getPositionsArea(positions: any) {
    let result = 0;
    if (positions) {
      let h = 0;
    ...
    if (this._viewer && options) {
      ...
      _handlers.setInputAction(function (movement) {
        ...
        if (cartesian && cartesian.x) {
          ...
          // 添加量测信息点
          $this._addInfoPoint(cartesian, positions,true);
  ...
  drawAreaMeasureGraphics(options = {}) {
    if (this._viewer && options) {
      ...
      _handler.setInputAction(function (movement) {
        ...
        // 添加信息点
        $this._addInfoPoint(positions[0], positions);
        if (typeof options.callback === "function") {
          options.callback(
            $this.transformCartesianArrayToWGS84Array(positions),
            polyObj
  ...
  //添加坐标点
  _addInfoPoint(position: any, positions: any,isPoint?:boolean=false): void {
    ...
    const text = isPoint ?( (
        ...
        ).toFixed(4) + "公里"):((this.getPositionsArea(this.transformCartesianArrayToWGS84Array(positions)) / 1000000.0).toFixed(4) + '平方公里')
    _labelEntity.label = {
      text,
      show: true,
      showBackground: true,
