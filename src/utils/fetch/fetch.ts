...
interceptors.response.use(
  async (response: any) => {
    ...
    if ((response.status == 401 || res.code == 401) && !response.requestConfig.withoutCheck) {
      ...
      if (!hash.includes('returnUrl')) window.location.href = pathname + '#/login?returnUrl=' + encodeURIComponent(hash.replace('#', ''));
    }
...
function request(method:string, path:string, data:{[prop:string]:any}, config:any = {}) {
  ...
  if (method === 'GET') {
    ...
    return Cfetch(`${path}${params ? '?' : ''}${params}`, {
      ...configDefault,
      ...config,
    });
  }
