...
class AddMarker {
  ...
  // }
  static mark = new Map()
  static add({ list=[], type='billbord', name='default', icon='', config, nodetail=false}){
    !icon && (icon = pointDefaultIcon);
    if(type === 'polygon'){
      return AddMarker.polygonFn({list, name, config, nodetail})
    }
    let markerType = new Cesium.BillboardCollection()
      switch(type){
        case 'polyline':
          markerType = new Cesium.PolylineCollection()
          break;
      }
      ...
      AddMarker.mark.set(name, {layers, data: listData})
  ...
  static polygonFn({list=[], name='default', config={}, nodetail=false}){ // list=[{points:[[lng,lat],[lng,lat]], pointsInset:[[[lng,lat],[lng,lat]]]}]
    ...
    AddMarker.mark.set(name, {polygons:layers, data: listData})
  }
  ...
  static get(names='', isData=false){
    if(!names || typeof names !== 'string') return;
    const [name, id] = names.split('->')
    let res = AddMarker.mark.get(name)
    if(res){
  ...
  static del(names=''){
    ...
    if(id){
      ...
      AddMarker.mark.delete(name)
    }
  }

  static show(name='', isShow){
    if(!name) return;
    let res = AddMarker.mark.get(name)
