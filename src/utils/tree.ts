...
/** 将 { id, name, children } 树转为 Naive UI TreeSelect options */
export function toNaiveTreeSelectOptions<T extends { id: any; name: string; children?: T[] }>(
  nodes: T[] = [],
): Array<{ key: any; label: string; children?: ReturnType<typeof toNaiveTreeSelectOptions> }> {
  return nodes.map(n => ({
    key: n.id,
    label: n.name,
    ...(n.children?.length ? { children: toNaiveTreeSelectOptions(n.children) } : {}),
  }))
}
