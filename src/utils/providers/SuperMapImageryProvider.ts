...
class SuperMapImageryProvider {
  ...
  constructor(options) {
    ...
    this._fRatio = defaultValue(
      options.ratio,
      Math.DEGREES_PER_RADIAN / 6378137.0
    );
    this._scales = [];
    this._coordUnit = "DEGREE";
    let credit = defaultValue(
      options.credit,
      new Credit("MapQuest, SuperMap iServer Imagery")
    );
  ...
  requestImage(x, y, level, request) {
    ...
    if (this._indexedDBSetting.isOpen) {
      if (defined(this._indexedDBScheduler)) {
        const promise = this._indexedDBScheduler.getElementFromDB(
          this.tablename,
          url
        );
...
const Scales = [
  1.690163571602655e-9, 3.3803271432053056e-9, 6.760654286410611e-9,
  1.3521308572821242e-8, 2.7042617145642484e-8, 5.408523429128511e-8,
  1.0817046858256998e-7, 2.1634093716513974e-7, 4.3268187433028044e-7,
  8.653637486605571e-7, 0.0000017307274973211203, 0.0000034614549946422405,
  0.0000069229099892844565, 0.000013845819978568952, 0.000027691639957137904,
  0.0000553832799142758, 0.0001107665598285516, 0.0002215331196571032,
  0.0004430662393142064, 0.0008861324786284128, 0.001772264957256826,
...
function onFulfilledRest3D(this, xmlText) {
  ...
  const { defaultValue, defined, GeographicTilingScheme, Math, Rectangle } =
    Cesium;
  ...
  this._rectangle.west < tilingScheme.rectangle.west &&
    (this._rectangle.west = tilingScheme.rectangle.west);
  this._rectangle.east > tilingScheme.rectangle.east &&
    (this._rectangle.east = tilingScheme.rectangle.east);
  this._rectangle.south < tilingScheme.rectangle.south &&
    (this._rectangle.south = tilingScheme.rectangle.south);
  this._rectangle.north > tilingScheme.rectangle.north &&
    (this._rectangle.north = tilingScheme.rectangle.north);
  ...
  const tileCount =
    (window.Math.abs(neTile.x - swTile.x) + 1) *
    (window.Math.abs(neTile.y - swTile.y) + 1);
  tileCount > 4 && (this._minimumLevel = 0);
  this._tilingScheme = tilingScheme;
  this._urlTemplate =
    this._url + "data/index/{y}/{x}.{fileExtension}?level={level}";
  this._ready = true;
...
function parseConfigFromXmlText(this, xmlText) {
  ...
  const levelsNodes =
    queryNodes(levelsNode, "Level", namespaceURI) || ([] as any);
  const levels: number[] = [];
  ...
  const fileExtentName = queryStringValue(
    rootNode,
    "FileExtentName",
    namespaceURI
  );
  const cellWidth = queryNumericAttribute(rootNode, "CellWidth", namespaceURI);
  const cellHeight = queryNumericAttribute(
    rootNode,
    "CellHeight",
    namespaceURI
  );
...
function queryFirstNode(xmlNode, attribute, namespaceURI) {
  if (Cesium.defined(xmlNode)) {
    ...
    for (let i = 0; i < length; i++) {
      const node = nodes[i];
      if (
        node.localName === attribute &&
        namespaceURI.indexOf(node.namespaceURI) !== -1
      ) {
        return node;
      }
    }
...
function onFulfilledTileMap(this, response) {
  ...
  if (
    defined(visibleScales) &&
    visibleScales.length > 1 &&
    defined(this._maximumLevel)
  ) {
  ...
  this._urlTemplate = this._urlTemplate.replace(
    "{transparent}",
    this._transparent
  );
  this.layersID &&
    (this._urlTemplate = this._urlTemplate + "&layersID=" + this.layersID);
  this._rectangle ||
    (this._rectangle = defaultValue(
      this._options.rectangle,
