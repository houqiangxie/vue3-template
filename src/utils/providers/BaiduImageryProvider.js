...
class BaiduImageryProvider {
    ...
    requestImage(x, y, level) {
        ...
        // let url = this._url.replace('{x}', x - xTiles / 2).replace('{y}', yTiles / 2 - y - 1).replace('{z}', level).replace('{s}', Math.floor(10 * Math.random()));
        if (this._crs === 'WGS84') {
        ...
        // const r = this._tilingScheme.getNumberOfXTilesAtLevel(level);
        // const c = this._tilingScheme.getNumberOfYTilesAtLevel(level);
        // const s = this.url.replace('{x}', x - r / 2).replace('{y}', c / 2 - y - 1).replace('{z}', level).replace('{s}', Math.floor(10 * Math.random()));
